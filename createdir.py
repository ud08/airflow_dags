...
create_dir = Variable.get("create_dir",deserialize_json = True)
# print(create_dir)
dirname = create_dir["dirname"]
hostname = create_dir["hostname"]
password = create_dir["password"]
username = create_dir["username"]

dag = DAG(
    'create_directory', schedule_interval='@once', default_args=default_args)
...
t1 = BashOperator(
  ...
  bash_command = '''curl --header "Authorization: Basic MjNiYzQ2YjEtNzFmNi00ZWQ1LThjNTQtODE2YWE0ZjhjNTAyOjEyM3pPM3haQ0xyTU42djJCS0sxZFhZRnBYbFBrY2NPRnFtMTJDZEFzTWdSVTRWck5aOWx5R1ZDR3VNREdJd1A=" -d '{ "hostname":"'''+hostname+'''", "username":"'''+username+'''", "password":"'''+password+'''", "port":"22", "dirname":"'''+dirname+'''"}' -H "Content-Type: application/json" -X POST https://192.168.60.12:31001/api/v1/namespaces/_/actions/created --insecure'''
  ,dag=dag)